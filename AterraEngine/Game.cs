...
public class Game {
    ...
    private async Task<bool> game_tick() {
        ...
        // print 
        for (int i = 0; i < area_rows; i++) { // Get the number of rows
            for (int j = 0; j < area_cols; j++) { // Get the number of columns

                string printable;

                Entity? entity = entity_area_grid[i, j];
                Tile tile = area.area_map[i, j];
                if (tile is POI poi && entity is not null) {
                    POI_buffer = poi;
                }
        ...
        if (POI_buffer is not null) {
            ...
            if (POI_buffer.area_link != null) {
                ...
                if (!entity_manager.TryAdd(area.area_id, new Dictionary<Entity, Vector2>(){{ player, new Vector2() }})) {
